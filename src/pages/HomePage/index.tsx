...
const HomePage: FC<HomePageProps> = ({ changeVersionRef }) => {
  ...
  const getLockouts = () => {
    ...
    const lockoutEnd = new Date(lockoutStart);
    lockoutEnd.setDate(lockoutEnd.getDate() + 7);
    const nextLockoutStart = new Date(lockoutEnd);
    nextLockoutStart.setDate(nextLockoutStart.getDate() + 7);
  ...
  useEffect(() => {
    BuildHelper.parseGetBuilds()
      .then((builds) => {
        loadRaids(builds, version ?? process.env.REACT_APP_DEFAULT_VERSION);
      })
      .catch(handleError);
