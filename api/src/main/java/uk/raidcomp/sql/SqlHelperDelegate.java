...
public class SqlHelperDelegate {
  ...
  public List<PlayerDto> getRoster(ImportRosterDto connectionString) {
    ...
    try {
      ...
      while (result.next()) {
        PlayerDto p = new PlayerDto(result.getString("id"), result.getString("name"), null,
            WarcraftPlayerClass.findByValue(result.getString("className")),
            WarcraftPlayerSpec.findByValue(result.getString("spec"), result.getString("className")),
            WarcraftPlayerRace.findByValue(result.getString("race")), InviteStatus.UNKNOWN, null,
  ...
  private String getConnectionString(SaveBuildDto Dto) {
    return "jdbc:mysql://" + Dto.server() + ":" + Dto.port() + "/" + Dto.database()
